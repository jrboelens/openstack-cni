...
func Test_CmdHandler(t *testing.T) {
	t.Run("can add and delete using a handler", func(t *testing.T) {
		WithTestConfig(t, func(cfg TestingConfig) {
			...
			port, err := deps.OpenstackClient().GetPortByTags(cniserver.NewPortTagsFromCommand(cmd).NeutronTags().AsStringSlice())
			Assert(t).That(err, IsNil())
			...
			port, perr := deps.OpenstackClient().GetPortByTags(cniserver.NewPortTagsFromCommand(cmd).NeutronTags().AsStringSlice())
...
func Test_NewPortTags(t *testing.T) {
	t.Run("ensure container ids are shortened to the first 12 chars", func(t *testing.T) {
		...
		tags := cniserver.NewPortTagsFromCommand(cmd).NeutronTags()
